...
parser.add_argument('--name_expe', default='mocov2', type=str)
parser.add_argument('--transformations', metavar='transfo', type=str, nargs="+",
                    default=['Hflip', 'Vflip'],
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        ...
        if args.multiprocessing_distributed:
            # For multiprocessing distoutpuributed training, rank needs to be the
    ...
    print(traindir)
    # normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], IMAGENET!!
    #                                std=[0.229, 0.224, 0.225])
    # normalize = transforms.Normalize(mean=[0.763, 0.602, 0.718], # norme challenge 512
    ...
    if args.aug_plus:
        ...
        augmentation = [
            ...
            transforms.RandomApply([
                transforms.ColorJitter(0.6, 0.6, 0.6, 0.1)  # not strengthened
            ], p=0.8),
            transforms.RandomGrayscale(p=0.5),
            # transforms.RandomApply([RandomHEaugmentation(0.03, 0.03)], p=0.7),
            transforms.RandomApply(
                [moco.loader.GaussianBlur([.05, 2.])], p=0.5),
            # RandomRotate90(),
        ...
        print('list of transformations: {}'.format(augmentation))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model, criterion, optimizer,
              scheduler, epoch, args, sum_writer, out_path)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best=False, filename=os.path.join(out_path, 'checkpoint_{:04d}.pth.tar'.format(epoch)))


def train(train_loader, model, criterion, optimizer, scheduler, epoch, args, sum_writer, out_path):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        # Writer update
        l2store = loss.detach().cpu().numpy().item()
